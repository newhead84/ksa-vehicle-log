...
- 전용차량 두 화면 모바일 최적화 및 다수 버그 수정 일괄 반영.
  ① 페이지 전반 롱프레스 시 텍스트 선택/콜아웃 메뉴 방지(input/textarea/select는 예외), 가로 스크롤 방지(`overflow-x:hidden`), 관리자 대시보드 상단바 좁은 화면 줄바꿈 처리.
  ② 기사님 입력화면 헤더 재구성: 첨부된 슬로건 이미지를 GENESIS 워드마크와 같은 너비로 위에 쌓고 옆에 "EXECUTIVE VEHICLE LOG" 텍스트 배치, 좌측 높이 증가에 맞춰 우측 사업장명(24→29px)·차량번호 칩(19→23px) 확대, 차량번호 칩 테두리를 연한 회색(`--ded-line`)에서 베이지(`--ded-beige`)로 강화.
  ③ 회장차 KSA직원 모드 선택 시 운행자명 입력창을 숨기지 않고 항상 노출하되 기본값('이왕희')만 지워 직접 입력받도록 변경(수행기사 모드 복귀 시 비어있으면 기본값 복원).
  ④ 운행기간 날짜 입력: 네이티브 달력 피커는 유지하면서 표시 텍스트만 YY-MM-DD 형식 오버레이로 전환(브라우저 로케일 포맷 YYYY. MM. DD. 잘림 문제 해결).
  ⑤ 명언 영역: 출처 미확인 note 문구 노출 제거, 명언 문구 자체는 클릭 불가로 전환하고 인물명에만 밑줄+클릭 시 해당 인물 구글 검색 기능 분리 적용.
  ⑥ 전무이사·KSA직원 모드에서 출발지/시작시각 입력창과 "출발 등록 저장" 버튼이 잘못 나타나던 버그 수정 — 차량/모드를 빠르게 전환할 때 이전 `checkQrPhase()` 비동기 응답이 늦게 도착해 화면을 덮어쓰는 레이스컨디션이 원인이었으며, 시퀀스 번호 기반 가드(`qrPhaseCheckSeq`)로 지연 응답을 무효화하도록 수정.
  ⑦ QR URL의 차량번호 노출 문제 해소 — `VEHICLES`에 임의 12자리 hex `token` 필드 추가, QR 링크를 `?plate=차량번호`에서 `?t=토큰` 방식으로 전환(구버전 QR 호환을 위해 `?plate=`는 폴백으로만 유지).
  ⑧ 전용차량 대시보드에 실 계약정보 반영 착수 — 차량별 `contractStartDate`(계약시작일) 필드 추가, "📋 계약정보 관리" 버튼/모달 신규(Firebase `dedicated-fleet-data/contracts`에 저장), 계약시작일 기준 총대여료 누계를 자동 계산해 상단 통계·차량별 카드에 표시. "⬇ 엑셀내보내기" 버튼 신규(SheetJS, 운행기록/계약·대여료요약 2개 시트).
  | 변경: dedicated-driver-input.html(CSS user-select/overflow-x/헤더·plate-chip/date-field 오버레이/quote-person 신규, JS KSA_SLOGAN_MARK 상수·setHoegjangMode()/refreshHoegjangLayout() 재작성·checkQrPhase() 시퀀스가드 추가·formatYYMMDD()/syncDateDisplays() 신규·pickRandomQuote() 재작성·applyQrLock() 토큰 지원, VEHICLES에 token 필드 추가), dedicated-admin-dashboard.html(xlsx 스크립트 추가, CSS user-select/overflow-x/topbar wrap/contract-row 신규, VEHICLES에 token·contractStartDate 필드 추가, fbPut()/loadContractsFromFirebase()/monthsElapsed()/totalRentalFor() 신규, openContractModal()/closeContractModal()/saveContracts()/exportTripsExcel() 신규, renderStats()/renderCards() 계약정보 반영하도록 수정, openQrModal() 토큰 기반 URL로 변경, #dedContractOverlay 모달 HTML 신규)

## 2026-08-07 (2)

- 회장차(177호1673) 이용자 구분 및 출발/도착 QR 이중화, 운행기간 달력 선택, 내 운행기록 확인/수정, 명칭 통일 등 전용차량 기사님 입력화면 고도화. ① 헤더 모델명에서 "/기본 4WD AT" 문구 삭제(제네시스 로고 우측에는 기존과 동일하게 협회명·차량번호만 노출, 전무이사차량과 동일 레이아웃). ② 회장차 화면 상단에 "🚗 수행기사 / 👤 KSA직원" 토글 신규 추가: KSA직원 선택 시 운행자명 입력을 숨기고(제출 시 driver='KSA직원' 자동 기록) 출발/도착지·시각·근태 블록을 숨겨 전무이사 입력화면과 동일한 레이아웃으로 전환. ③ 회장차+수행기사 모드 전용으로 출발/도착 QR 이중화 구현(QR 1개 재사용, 수행기사 OJT 교육으로 당일 출발시 1회·도착시 1회 스캔): 페이지 진입 시 Firebase `dedicated-fleet-data/vehicles/{plate}/dailyState/{오늘날짜}` 조회해 출발기록 유무로 출발/도착 단계 자동 판별 → 출발 단계에서는 출발지·시작시각·시작계기판만 입력받아 dailyState에 부분 저장(운행기록 미생성), 도착 단계에서는 도착지·종료시각·종료계기판·주유비 등을 입력받아 dailyState와 병합해 최종 운행기록(trips)으로 저장 후 dailyState 삭제. ④ 시작 시각/종료 시각 입력 시(자동입력 포함) 근로 시작/종료 시간에 동일 값 즉시 연동(oninput 연결, syncWorkTime()). ⑤ 운행기간 입력을 숫자 마스킹 텍스트input에서 `<input type="date">` 달력 선택으로 전환(전 차량 공통). ⑥ 하단에 "📋 내 운행기록 확인/수정" 패널 신규 추가 — 해당 차량 최근 10건을 Firebase에서 불러와 목록 표시, 항목 클릭 시 해당 기록 전체를 폼에 불러와 수정 후 재저장(trips 배열 해당 인덱스 덮어쓰기, submittedAt 보존·editedAt 기록), 상단에 수정중 배너와 취소 버튼 제공. ⑦ '전무(본인)' 명칭을 '전무이사(본인)'로, 프리뷰 전환용 셀렉트 옵션의 '전무차'를 '전무이사차'로 통일(dedicated-admin-dashboard.html의 VEHICLES role도 동일하게 통일) | 변경: dedicated-driver-input.html(HTML: #hoegjangModeBar/#qrPhaseBanner/#driverNameRow/#myRecordsBtn·#myRecordsPanel·#myRecordsList/#editBanner 신규, 운행기간 input을 type=date로 변경, CSS .ded-mode-toggle/.ded-mode-btn/.ded-qr-phase-banner/.ded-myrecords-*/.ded-edit-banner 신규; JS: switchVehicle() 재작성, setHoegjangMode()/refreshHoegjangLayout()/checkQrPhase()/enterDepartPhase()/enterArrivePhase()/todayIso()/nowHHMM()/syncWorkTime()/toggleMyRecords()/loadMyRecords()/startEditRecord()/cancelEditMode() 신규, submitLog() 출발/도착/수정 분기 처리로 전면 재작성, initDates() type=date 대응, VEHICLES.hoegjang.modelFull 문구 삭제, role 전무이사 통일, formatDateMask()/maskedDateToIso() 제거), dedicated-admin-dashboard.html(VEHICLES role 전무이사 통일)
- 전용차량 관리자 대시보드에 QR/링크 발급 기능 추가: 기존에는 관리자 대시보드만 볼 수 있고 기사님 입력 페이지(dedicated-driver-input.html) URL을 확인할 방법이 없었음. 각 차량 카드에 "📱 QR / 링크 보기" 버튼을 추가해, 클릭 시 index.html의 QR 발급 기능(openQrModal 등)과 동일한 패턴으로 QRCode.js 기반 QR코드를 생성. QR은 `dedicated-driver-input.html?plate=차량번호` URL을 담아 스캔 시 해당 차량으로 자동 고정(기존 구현한 applyQrLock 활용). 모달에서 링크 복사/새 탭 열기/인쇄(카드 규격 63.25×90mm, 공용차량과 동일)/PNG 저장 지원 | 변경: dedicated-admin-dashboard.html(qrcodejs 스크립트 추가, DEDICATED_APP_URL 상수 신규, #dedQrOverlay 모달 HTML 신규, openQrModal()/closeQrModal()/copyDriverLink()/openDriverLink()/printQrDedicated()/downloadQrDedicated() 신규, renderCards()에 QR 버튼 추가, CSS .qr-btn/.ded-qr-* 신규)
- 전용차량 두 화면(dedicated-driver-input.html, dedicated-admin-dashboard.html)의 프로토타입 상태(정적 데모 데이터) 해소 — Firebase 실데이터 연동. index.html과 동일한 Firebase 프로젝트/익명인증 방식(firebase-app-compat, firebase-auth-compat, REST API fbGet/fbSet)을 적용하되, 공용차량 데이터(fleet-data)와는 완전히 분리된 별도 노드 `dedicated-fleet-data/vehicles/{차량번호}/trips`를 사용. ① 기사님 입력화면: submitLog()를 async로 전환해 제출 시 최신 데이터를 재조회·병합 후 Firebase에 저장(동시 제출 유실 방지), 실패 시 에러 토스트, 성공 시 폼 초기화, 운행기간/계기판 최소 검증 추가. ② QR 잠금(`?plate=` 차량번호 파라미터) 신규 구현 — URL 파라미터와 일치하는 차량으로 자동 고정하고 PREVIEW 전환 UI 숨김(실사용 QR코드 부착 시나리오 대응). ③ 관리자 대시보드: 정적 TRIPS/monthDays/monthKm 목데이터를 제거하고 Firebase에서 실시간 조회해 이번 달 운행일수·주행거리를 동적 계산, 로그인 시 자동 로드 + ⟳ 새로고침 버튼 추가, 로드 실패 시 배너 표시. ACCESS_LOG(접속기록)는 저장 기능이 없어 데모 데이터로 유지(범위 밖, 별도 요청 시 진행) | 변경: dedicated-driver-input.html(Firebase 설정/인증/fbGet·fbSet 신규, applyQrLock() 신규, submitLog() 전면 재작성, collectPeriods()/maskedDateToIso()/showToast()/resetFormAfterSubmit() 신규), dedicated-admin-dashboard.html(Firebase 설정/인증/fbGet 신규, loadTripsFromFirebase()/monthlyStatsFor()/refreshDashboard() 신규, attemptAuth() async 전환, renderCards() 동적 통계 적용, HTML #refreshDataBtn 신규)
- 늦은입력 자동보정 로직에 날짜 검증 보강: 시작거리가 겹치는 것(수치 조건)만으로는 자동보정하지 않고, A의 운행기간 마지막 날(dateTo, 없으면 dateFrom)이 B의 시작일(isoDate)보다 이후이면(=시간 순서가 맞지 않으면) 자동보정 대상에서 제외하고 "확인필요" 경로로 전환하도록 수정. 시작거리 우연 일치 + 날짜 역전 조합(오타 등 다른 오류 가능성)에 자동보정이 잘못 실행되는 것을 방지 | 변경: resolveLateEntryInsert()(dateOrderOk 조건 추가), notifyLateEntryAdjustment()/openVehicle()(확인필요 안내문구를 거리역전·날짜순서 두 사유 모두 아우르도록 일반화)
- 이전 사용자(A)가 QR 운행정보 입력을 깜박해 다음 사용자(B)가 먼저 입력하면 두 사람분 거리가 B의 기록 한 건에 합산되는 문제를 자동으로 바로잡는 기능 추가. 평균거리 비교 방식은 공용차량 특성상(사용자마다 운행거리가 제각각) 의미가 없다고 판단해 채택하지 않고, 실제 시작/종료거리 체인이 겹치는 지점을 직접 찾아 보정하는 방식으로 구현: ① A가 뒤늦게 입력하면, A의 시작거리와 동일한 시작거리를 가진 기존 B의 기록을 찾아 B의 시작거리를 A의 종료거리로 자동 갱신하고 B의 주행거리를 재계산(원본 시작거리·주행거리는 originalStartOdo/originalDistance로 보존 — 무손실 원칙), ② 자동 보정 시 B의 기록에 "🔧 자동보정됨" 배지 표시(마우스오버 시 보정 전 값 확인), ③ A의 종료거리가 B의 종료거리 이상이라 자동 보정 시 거리가 역전되는 경우는 값을 그대로 두고 A의 기록에 "⚠ 확인필요" 배지만 표시(관리자 수동 확인 유도), ④ 자동보정 또는 확인필요 건이 발생하면 기존 점검요망 알림과 별도의 EmailJS 이메일을 관리자에게 발송(어느 기록이 어떻게 보정됐는지 상세 포함) | 변경: resolveLateEntryInsert()(신규), notifyLateEntryAdjustment()(신규), doSubmit()(트립 저장 시 체인 겹침 판정·보정 호출, 알림 호출), openVehicle()(운행기록 표 자동보정/확인필요 배지), CSS(.lateadjust-badge/.lateconflict-badge 신규)
- 전용차량(회장차·전무차 2대) 운행정보 대시보드/기사님 입력화면 신규 연동. 두 화면은 공용차량 시스템과 완전히 분리된 별도 디자인 언어(제네시스 톤 블랙·베이지, DEDICATED-VEHICLE-DESIGN-GUIDE.md 참조)로 제작되어 index.html과 별개 파일(dedicated-admin-dashboard.html, dedicated-driver-input.html)로 유지하고, index.html에는 관리자 상단 툴바 진입 버튼만 추가(새 창으로 열림). 열람전용 URL(전체/지역별)에는 화이트리스트 방식(VIEWER_ALLOWED_TOOLBAR_BTN_IDS)에 의해 신규 버튼이 자동으로 노출되지 않음(추가 조치 불필요) | 변경: HTML(#dedicatedVehicleBtn 신규), 신규 파일 dedicated-admin-dashboard.html/dedicated-driver-input.html 추가
...
- ★가동율 계산 오류 버그 수정(중요): iso() 함수가 UTC 변환(toISOString())을 사용해 KST 자정 기준 Date를 하루 이른 날짜로 잘못 변환하던 버그를 수정. 이 버그는 doSubmit()(일반 QR 제출)·saveTripEdits()(관리자 수정)·saveUserEdits()(본인수정)·applyTripRecordImport()(전체기록 업로드) 등 운행일자를 생성하는 모든 경로에 존재했으며, isoDate/dateTo(화면 표시용)는 정상 저장되지만 usageDates(가동율 계산용)만 하루 이르게 저장되어 화면에 보이는 날짜는 맞는데 가동율만 실제보다 낮게 계산되는 현상이 있었음(발견 계기: 차량 183호5250 가동율이 50%로 표시되어 역추적, 8/3·8/5 이틀 운행이면 66.7%가 나와야 하나 8/3이 usageDates 계산상 8/2(일요일)로 밀려 업무일에서 제외됨). iso()를 localDateKey()와 동일한 로컬(KST) 기준 계산으로 교체해 모든 호출부를 한 번에 정상화. ② 기존에 이미 잘못 저장된 usageDates를 복구하는 1회성 관리자 도구(🛠 usageDates 복구(1회성)) 신규 추가 — isoDate~dateTo 기준으로 재계산 후 어긋난 건수만 점검·확인(confirm)받아 저장, 복구 완료 후에는 버튼/함수 제거 가능 | 변경: iso()(localDateKey 기반으로 교체), computeCorrectUsageDates()/auditUsageDatesDrift()/repairUsageDatesDrift()(모두 신규), HTML(#repairUsageDatesBtn 신규)
- "사업장별 차량 가동율 운행거리 비교차트"에서 사업장 막대를 눌러 펼친 소속 차량 상세 목록 개선: ① 차량번호 우측에 차종명(그랜저/소나타/아반떼) 병기, ② 기존에는 가동율·운행거리 값 기준으로 정렬되어 차종이 뒤섞여 보이던 것을 차종 정렬원칙(그랜저>소나타>아반떼, 동일 차종은 차량번호순)으로 통일 — 가동율/운행거리 두 지표 모두 동일하게 반영. 병기 텍스트가 들어갈 여유 확보를 위해 사업장별 모드 라벨 폭도 소폭 확대 | 변경: renderMonthlyCompareChart()(subVehicles 정렬 기준, 소속 차량 라벨, labelW)
- 차종 정렬 규칙(사업장→차종[그랜저/소나타/아반떼순]→차량번호) 적용 범위 확장: ① 엑셀 내보내기(exportToExcel)의 차량 정렬 기준에 기존 modelOrder()가 누락되어 있어 사업장 내에서 차종 순서가 뒤섞여 보이던 문제 수정(전체차량현황·전체운행기록·월별가동율 시트 모두 동일 정렬 반영), ② 1~7월 수기기록 업로드 샘플양식(downloadManualMonthlyTemplate)에 '차종' 열이 없던 것을 추가(참고용 표시 전용, 파싱 로직 미변경)하고 정렬도 동일 기준으로 통일, 예시행 2건에도 차종 값 반영 | 변경: exportToExcel()(plates 정렬), downloadManualMonthlyTemplate()(metaCols/plates 정렬/rows/example1,2/열폭)
- ① 지역구분별 가동현황 표 "운행건수" 헤더의 "(개인사용제외)" 부기 설명 삭제(단순 표기로 환원), ② 상단 요약카드(statStrip) "총 누적 운행거리"·"누적 운행 건수"·"누적 주유비" 3개 카드에 개인사용(isPersonal) 누적 데이터를 각 카드 우측에 작게 병기(예: "개인 1,234km") — 별도 카드 신설 대신 기존 카드 내 값 옆에 표시하는 방식으로 결정. 출퇴근사용(isCommute)은 개인사용과 별개 항목이라 이번 병기 대상에서 제외(필요 시 기존 "운행자별 현황" 표의 개인사용건수/출퇴근사용건수 컬럼에서 확인 가능) | 변경: renderRegionSummary() 헤더 텍스트, renderStats()(personalDist/personalFuel/personalTrips 집계 추가)

## 2026-08-04

- 대시보드 상단 요약카드(총 누적 운행거리·누적 주유비)와 차량별 카드(주행거리·주유비)를 업무용 기준(개인사용 isPersonal·출퇴근사용 isCommute 제외)으로 집계하도록 변경 — 대시보드가 업무용 차량관리 목적임을 반영, 가동율 계산과 동일 기준으로 통일. 차량상세 모달·엑셀 내보내기(총괄자료)는 기존과 동일하게 개인/출퇴근사용 포함 전체 데이터 유지(법인세 증빙 등 총괄자료 용도) | 변경: renderStats(), vehicleStatsOf()(businessOnly 파라미터 신규), vcardHtml()
