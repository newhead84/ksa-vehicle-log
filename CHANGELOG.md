...
- 가동율 계산 불일치 버그 수정: 차량상세 모달 상단 "가동율" 요약과 지역구분별 가동현황 표의 가동율이 "최초 운행일~최종 운행일 사이의 업무일수"를 분모로 계산되어, 최근 재배치된 차량처럼 운행기록이 짧은 기간에 몰려있으면 실제로는 대부분 개인사용/출퇴근이어도 100%에 가깝게 표시되던 문제(예: 181호6040 8/3~4가 개인사용·출퇴근인데도 상단 가동율 100% vs 바로 아래 월별표 8월 10%로 서로 다르게 표시) 수정. 차량상세 모달은 월별표(연평균)와 동일한 "유효 월 평균" 방식으로, 지역구분별 표는 "월별 업무일수 대비 비율의 평균" 방식으로 변경해 항상 서로 일치하는 값을 보여주도록 통일. 월별 가동율 표·비교차트(연평균)는 이미 올바른 방식이었으므로 변경 없음 | 변경: plateAnnualAvg()(신규), monthlyAvgRateFromDates()(신규), openVehicle()(util 계산을 plateAnnualAvg()로 교체), renderRegionSummary()(rate 계산을 monthlyAvgRateFromDates()로 교체)
